...
func sendTxs(ctx context.Context, cs *State) {
	for i := 0; i < 256; i++ {
		...
		default:
			tx := []byte{byte(i)}
			err := assertMempool(cs.txNotifier).CheckTx(ctx, tx, nil, mempl.TxInfo{})
			if err != nil {
				panic(err)
			}
			i++
...
func TestSimulateValidatorsChange(t *testing.T) {
	...
	err = assertMempool(css[0].txNotifier).
		CheckTx(ctx, newValidatorTx2, nil, mempl.TxInfo{})
	assert.Nil(t, err)
	...
	err = assertMempool(css[0].txNotifier).
		CheckTx(ctx, removeValidatorTx2, nil, mempl.TxInfo{})
	assert.Nil(t, err)
